...
for (let i = 0; i < arr1.length ; i++) {               
    if (arr1[i] !== num1) {        // if the index does not equal num1 then 
      arr1Result.push(arr1[i]);    // push non-num1 values to empty array -> arr1Result
    }
  }
...
/**
 * Q3: Write code to find the index of second-occurrence of 'baSeBalL' (ignoring cases) in the arr3-array
...
// code
let pattern = 'baSeBalL'
let pattern_LowerCase = pattern.toLowerCase();

for (let i = 0; i < arr3.length; i++) {                 
    arr3[i] = arr3[i].toLowerCase();           // converts all indexes to lowercase
}
